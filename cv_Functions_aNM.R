...
###########################################################################################################
# Weighting procedure according to 3 methods : 
# 1) aNM (Vagnon et al. 2021)
# 2) aNM2 (unpublished)
# 3) Bartomeus (Bartomeus et al. 2016;Reused by Pomeranz et al. 2020)
#/!\ if method used is "aNM2", the Rpackage "scales" must be loaded before
#
# Inputs : 
...
# method = can be "aNM","aNM2","Bartomeus"

# If method = "Bartomeus you have to provide: 
# params = params obtained from the function fit_it.R of Bartomeus (2016) must be provided
# Category = species taxnomique category == "vertebrate"or "vertebre" or "invertebrate"or "invertebre" 
# 
# Returns : the weighted interaction matrix according to the method choosen

Weighting<-function(Niche_attributes,Bmat,method,params,Category){
  
  if(method=="aNM"){
    for(i in 1:ncol(Bmat)){
      if(length(which(Bmat[,i]==1))!=0){
        
        Bmat[which(Bmat[,i]==1),i]<-dnorm(Niche_attributes$n[which(Bmat[,i]==1)],
                                          mean=Niche_attributes$c[i],
                                          sd=sd(seq(from=Niche_attributes$low[i],to=Niche_attributes$high[i],
                                                    by=((Niche_attributes$high[i]-Niche_attributes$low[i])/100))))/
                                    max(dnorm(Niche_attributes$n[which(Bmat[,i]==1)],
                                              mean=Niche_attributes$c[i],
                                              sd=sd(seq(from=Niche_attributes$low[i],to=Niche_attributes$high[i],
                                                     by=((Niche_attributes$high[i]-Niche_attributes$low[i])/100)))))
  ...
  if(method=="Bartomeus"){   
    for(i in 1:ncol(Bmat)){
      if(length(which(Bmat[,i]==1))!=0 & Category [i]=="vertebrate" |
         length(which(Bmat[,i]==1))!=0 & Category [i]=="vertebre"){
         optimum = params[[1]][[1]] + params[[1]][[2]]*Niche_attributes$n[i]
         range = params[[1]][[3]] + params[[1]][[4]]*Niche_attributes$n[i]
         
         Bmat[which(Bmat[,i]==1),i]<-exp(-(optimum-Niche_attributes$n[which(Bmat[,i]==1)])^2/(2*range)^2)
        
      }
      if(length(which(Bmat[,i]==1))!=0 & Category [i]!="vertebrate" |
         length(which(Bmat[,i]==1))!=0 & Category [i]!="vertebre"){
        optimum = params[[2]][[1]] + params[[2]][[2]]*Niche_attributes$n[i]
        range = params[[2]][[3]] + params[[2]][[4]]*Niche_attributes$n[i]
...
make_bern<-function(n,Wmat){
  ...
  for(k in 1:n){
    ...
    for (i in 1:ncol(Wmat)){
      for (j in 1:nrow(Wmat)){
